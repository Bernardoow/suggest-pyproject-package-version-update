...
describe("discoveryDependencySpecification Tests", () => {
    ...
    describe("Inequality requirements", () => {
        each([">=", ">", "<", "<=", "!="]).test(
            ...
            (inequality: string) => {
                expect(
                    discoveryDependencySpecification(`${inequality}2.24.0`),
                ).toEqual(inequality);
            },
        );
    });
...
describe("processDependencyList Tests", () => {
    ...
    each([
        ...
        (dependency: string, version: string) => {
            ...
            expect(
                updatedPyProjectFileToml.tool.poetry["dependencies"][
                    dependency
                ],
    ...
    each([
        ...
        (dependency: string, version: string) => {
            ...
            expect(
                updatedPyProjectFileToml.tool.poetry["dev-dependencies"][
                    dependency
                ],
            ).toStrictEqual(version);
        },
...
describe("processPyProjectAndVersion Tests", () => {
    ...
    test("It should be equal a dependencies", () => {
        const result = processPyProjectAndVersion(
            pyProjectFileToml,
            versionsData,
        );
        ...
        expect(result.tool.poetry.dependencies).toMatchObject(
